...
<template>
  <div class="projects-page">
    <PageHeader 
      title="🏗️ Proje Yönetimi"
      subtitle="Proje Bazlı Malzeme Takibi ve Rezervasyon Sistemi"
      item-type="Proje"
      export-label="Proje Raporu"
      @add-item="openCreateProjectModal"
    ...
    <div class="project-filters">
      ...
      <div class="filter-group">
        ...
        <input 
          v-model="searchText" 
          type="text" 
          placeholder="Proje adı, müşteri ara..."
        >
      </div>
    </div>
    ...
    <div class="projects-grid">
      <div v-for="project in filteredProjects" :key="project._id" class="project-card">
        ...
        <div class="project-stats">
          <div class="stat-item">
            <span class="stat-value">{{ project.totalItems }}</span>
            <span class="stat-label">Toplam Malzeme</span>
          </div>
          <div class="stat-item">
            <span class="stat-value" :class="getStockSufficiencyClass(project.stockSufficiency)">
              %{{ project.stockSufficiency }}
            </span>
            <span class="stat-label">Stok Yeterliliği</span>
          </div>
          <div class="stat-item">
            <span class="stat-value">{{ project.reservedItems }}</span>
            <span class="stat-label">Rezerve</span>
          </div>
          <div class="stat-item">
            <span class="stat-value">{{ formatCurrency(project.totalMaterialCost) }}</span>
        ...
        <div class="stock-status-bar">
          <div class="status-segment available" 
               :style="{ width: getAvailablePercentage(project) + '%' }">
          </div>
          <div class="status-segment reserved" 
               :style="{ width: getReservedPercentage(project) + '%' }">
          </div>
          <div class="status-segment missing" 
               :style="{ width: getMissingPercentage(project) + '%' }">
          </div>
        </div>
        <div class="status-legend">
          <span class="legend-item available">
            ✅ Mevcut: {{ project.availableItems }}
          </span>
          <span class="legend-item reserved">
            🔒 Rezerve: {{ project.reservedItems }}
          </span>
          <span class="legend-item missing">
            ❌ Eksik: {{ project.missingItems }}
          </span>
        </div>
        ...
        <div class="project-actions">
          ...
          <button 
            ...
            @click="reserveMaterials(project)"
            :disabled="project.availableItems === 0"
...
import type { Project } from '../types/projects' // Changed from '../types/project'
...
const reserveMaterials = async (project: Project) => {
  if (confirm(`"${project.name}" projesinin malzemelerini rezerve etmek istediğinizden emin misiniz?`)) {
    try {
      await projectStore.reserveProjectMaterials(project._id!)
...
.stat-value.sufficient { color: #10b981; }
.stat-value.partial { color: #f59e0b; }
.stat-value.insufficient { color: #ef4444; }
...
.status-segment.available { background: #10b981; }
.status-segment.reserved { background: #f59e0b; }
.status-segment.missing { background: #ef4444; }
